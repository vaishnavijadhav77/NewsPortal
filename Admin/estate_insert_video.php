...
<body>
<div class="container">
<div class="header"  style="padding:15px;font-size:25px;text-align:center;background-color:#343a40!important"><h2 style="color:white">Real Estate Video</h2></div>
  <div class="header"><h4 align="left" style="padding:5px;background-color:white"><span><i class="fas fa-home" style="margin:5px"></i></span><a href="index.php">Home > <a href="index.php?viewrv=viewrv">All Real Estate Videos</a><h3 align="right" ><a href="logout.php"><h3 style="color:red;margin-right:15px"><button style="font-size:24px;color:red;cursor:pointer">Admin Logout <span><i class="fas fa-sign-out-alt"></i></button></h3></a></h4></h3></div>
  <div class="row">
   <div class="col-sm-2" style="background-color:#c7ccd1">
   
   </div>
   <div class="col-sm-8">
<form action="estate_insert_video.php" method="post" enctype="multipart/form-data">
...
$query = "insert into realestate_video(id,title,link,descr,date) values('$id','$title','$link','$desc','$date')";
	mysqli_set_charset($conn,'utf8');

    if(mysqli_query($conn,$query)) 
    {
        echo "<script>alert('New Video Has Been Inserted..')</script>";
        echo "<script>window.open('index.php?viewrv=viewrv','_self')</script>";
    }
